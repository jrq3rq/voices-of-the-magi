...
const GROK_API_URL = "https://api.x.ai/v1/chat/completions"; // ← NEW correct endpoint (updated)
const GROK_API_KEY = process.env.GROK_API_KEY;

if (!GROK_API_KEY) {
  console.log("Error: GROK_API_KEY not found in .env");
  ...
  cors(req, res, async () => {
    ...
    try {
      const response = await fetch(GROK_API_URL, {
        ...
        body: JSON.stringify({
          model: "grok-3", // or "grok-beta" if you prefer
          temperature: 0.7,
          max_tokens: 2048,
          messages: [
      ...
      if (!response.ok) {
        ...
        return res.status(response.status).json({ reply: "The stars are silent right now. Please try again later." });
      }

      const data = await response.json();
      let reply = data.choices?.[0]?.message?.content || "No response from the Magi.";
      return res.json({ reply });
    } catch (error) {
